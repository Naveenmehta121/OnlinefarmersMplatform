-- Create the database for the online marketplace
...
-- Roles table (for managing user roles)
CREATE TABLE Roles (
...
-- Adding a role to Users table (foreign key relationship)
...
-- Products table (products listed by sellers)
...
-- Addresses table (for managing multiple addresses)
...
-- Orders table (stores the orders placed by customers)
CREATE TABLE Orders (
...
-- Order_Items table (detailed items within each order)
...
-- Payments table (payment details for an order)
...
-- Reviews table (customer reviews on products)
